...
class GAN(object):
	...
	def __init__(self):
		...
		self.g_W2 = np.random.randn(512,784).astype(np.float32)# * np.sqrt(2.0/(512))
		self.g_b2 = np.zeros(784).astype(np.float32)
		
		# self.g_W3 = np.random.randn(1024,28*28).astype(np.float32) * np.sqrt(2.0/(1024))
		...
		# self.g_w3_m, self.g_w3_v = 0.0, 0.0
		# self.g_b3_m, self.g_b3_v = 0.0, 0.0
	...
	def discriminator(self, img):
		...
		self.d_h1_l = instance_norm(self.d_h1_l)		
	...
	def generator(self, z):
		...
		self.g_h1_l = instance_norm(self.g_h1_l)
		self.g_h1_a = lrelu(self.g_h1_l)
		
		self.g_h2_l = self.g_h1_a.dot(self.g_W2) + self.g_b2
		self.g_h2_a = tanh(self.g_h2_l)

		# self.g_h3_l = self.g_h2_a.dot(self.g_W3) + self.g_b3
		# self.g_h3_a = tanh(self.g_h3_l)

		self.g_out = np.reshape(self.g_h2_a, (self.batch_size, 28, 28))
	...
	def backprop_gen(self, fake_logit, fake_output, fake_input):
		...
		# grad_W3 = self.g_h2_a.T.dot(loss_deriv)
		# grad_b3 = np.mean(loss_deriv, axis=0)		
		# prev_layer = np.expand_dims(self.g_h2_a, axis=-1)
		# loss_deriv_ = np.expand_dims(loss_deriv, axis=1)
		# grad_W3 = np.mean(np.matmul(prev_layer,loss_deriv_), axis=0)
		# grad_b3 = np.mean(loss_deriv_, axis=0)

		# loss_deriv = loss_deriv.dot(self.g_W3.T)
		# loss_deriv = loss_deriv*lrelu(self.g_h2_l, derivative=True)
		# grad_W2 = self.g_h1_a.T.dot(loss_deriv)
		...
		# grad_W1 = self.g_h0_a.T.dot(loss_deriv)
		...
		# #g_W3
		# self.g_w3_m = (beta1 * self.g_w3_m) + (1.0 - beta1) * grad_W3
		# self.g_w3_v = (beta2 * self.g_w3_v) + (1.0 - beta2) * (grad_W3 ** 2)
		# #g_b3
		# self.g_b3_m = (beta1 * self.g_b3_m) + (1.0 - beta1) * grad_b3
		# self.g_b3_v = (beta2 * self.g_b3_v) + (1.0 - beta2) * (grad_b3 ** 2)
		...
		# self.g_W3 = self.g_W3 - self.learning_rate*(self.g_w3_m/(np.sqrt(self.g_w3_v)+epsilon))
		# self.g_b3 = self.g_b3 - self.learning_rate*(self.g_b3_m/(np.sqrt(self.g_b3_v)+epsilon))
	...
	def backprop_dis(self, 
		...
		# grad_real_W2 = self.d_h1_a.T.dot(loss_deriv)
		...
		loss_deriv = loss_deriv.dot(self.d_W2.T)
		loss_deriv = loss_deriv*lrelu(self.d_h1_l, derivative=True)
		# grad_real_W1 =  self.d_h0_a.T.dot(loss_deriv)
		...
		# grad_fake_W2 = self.d_h1_a.T.dot(loss_deriv)
		...
		loss_deriv = loss_deriv.dot(self.d_W2.T)
		loss_deriv = loss_deriv*lrelu(self.d_h1_l, derivative=True)
		# grad_fake_W1 = self.d_h0_a.T.dot(loss_deriv)
